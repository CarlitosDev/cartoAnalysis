function plotAnalysisOverMeshInteractiveGuiData( meshData, cartoData, cartoPointAnalysis, pointDescriptor )
...
if enableInteraction
  ...
  for pointIdx = 1:numel(xData)
   ...
   % setappdata(plotHandle(pointIdx), 'cartoAnalysis', cartoPointAnalysis{pointIdx});    
    guidata(plotHandle(pointIdx), cartoPointAnalysis{pointIdx});    
    set(plotHandle(pointIdx), ...
        'HitTest', 'on', ...
        'ButtonDownFcn', @(~,~) plotCartoAnalysisCallbackGUI(plotHandle(pointIdx)));  
