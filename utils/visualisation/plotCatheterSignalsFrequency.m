function plotCatheterSignalsFrequency(cathTable, currentPoint, currentType)

% PLOTCATHETERSIGNALSFREQUENCY plot Pentaray spatial, temporal and
% frequencial.
%
...
%%
  ...
  for iChunk = 1:numel(intA)
    ...
    plot3(lAx,x,y,z, 'Color', [0,0,0], 'LineWidth', 2);
  end
...
%     text(rAx1, idx, timeAxes(end-10), ...
%       mean(currentSignal(end-200:end-100)), ...
%       subSet{idx, 'pentaRayElectrode'});
...
%     text(rAx2, idx, currentFreqAxes(10), ...
%       currentFreqValues(10), ...
%       subSet{idx, 'pentaRayElectrode'});
