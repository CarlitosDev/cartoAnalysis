...
% All points recorded with the catherer
listOfPoints = unique(cathTable{:, 'pentaRayPoint'});
currentPoint = listOfPoints(54);
currentType  = 'BIP';

% plotCatheterSignals(cathTable, currentPoint, currentType);

plotCatheterSignalsFrequency(cathTable, currentPoint, currentType);
...
[btFrequency,indff,bwfromf0,z,periodogramInput,fz] = df_Ng(dataForICA, selDataIn.fs);
% deltaFreq = fz(2)-fz(1);
% ancho = 2*deltaFreq;
ancho = 1.8;%1Hz
[oi,ejexplot,ejeyplot] = organizationIndex(periodogramInput,fz,btFrequency,ancho);
...
botteronFrequency(idxPoint) = btFrequency;
...
if doSaveProcessed 
  processedDataFolder = fullfile(getDataFolder(), ...
                        'preprocessed data');
  currentMatFile      = ['PreProcessed_', srcDataName, getTimeStamp,'.mat'];
  fullPath2MatFile    = fullfile(processedDataFolder, currentMatFile);

  vars2save           = {'cartoPointAnalysis', 'botteronFrequency', ...
                         'meshData', 'cartoData', 'cathererData', ...
                         'pointsInfo', 'pointsPosition', 'testType', ...
                         'cartoFile', 'samplingFrequency', 'lengthSignal'};
