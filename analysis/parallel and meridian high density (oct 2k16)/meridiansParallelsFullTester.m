...
for iSignal = 1:15   
    inputSignalName     = pentaArray.pentaBipolar{iSignal};
...
plotsFolder = fullfile(rootFolder(), '..', 'parallelAndMeridianPlots');
mkdir(plotsFolder);
...
%% Run a massive test different points

numPointsToTest = 20;
numPoints = numel(pointsInfo.allPointsInfo);

idxRandomPoints = randi(numPoints, 1, numPointsToTest);

testingPoints = pointsInfo.allPointsInfo(idxRandomPoints);


for idx = 1:numPointsToTest
   
    selectedPoint = testingPoints(idx);   
    ...
    % add the paths
      ...
      validNumPoints = pathInfo.numMeridianPoints > 0 & ...
                       pathInfo.numParallelPoints > 0 ;
       ...
       if validNumPoints
         ...
         figTitle = sprintf('%s_point_%d', ...
           ...
           selectedPoint);
         saveFigureAsPNG(figTitle, plotsFolder);
