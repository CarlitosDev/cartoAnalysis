...
%   Load mat file and dump into workspace
  ...
  preprocessedDataFileName = 'Results FA_RamonYCajal ECG.mat';

  matchingFNames = which(preprocessedDataFileName);
  if isempty(matchingFNames)
    errordlg('Can''t resolve path to file. Run initialise()');
    return;
  end

  preprocessedData = load(matchingFNames);

  fNames = fieldnames(preprocessedData);
  for idx=1:numel(fNames) 
    assignin('base', fNames{idx},  preprocessedData.(fNames{idx}));
  end
...
%% Pick a point to research.
  ...
  preprocessedData = cell2mat(ecgData.signal');
  
  %basicStats = Utils_basic_stats( preprocessedData );
  
  [inputDataSorted, meanEnergyValues, idxNRG] = sortDataByMeanEnergy(preprocessedData);
