...
for currentIdx = 1:numSignals
    ...
    currentSignal     = preprocessData(currentIdx, :);
    %currentSignal     = scaleFrom0To1(currentSignal);
    ...
    [inputPSD, inputPSDOmega] = ...
      ...
      'lengthFFT' , 8192, 'numberOverlap', [], 'psdCutOff', f3LowPassCutOff);
  
  
      if subPlotIdx==0
        subplot(numSignals, 2, 3),
        plot(currentSignal, 'Color', [0 1 0], 'LineWidth', 1.5);
      end
    ...
    signalTitle = sprintf('Processed %s - point %d', currentSignalName, currentPoint);
